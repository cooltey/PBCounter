...
interface RecordDao {
    ...
    @Query("SELECT SUM(level_1) AS level_1_total, SUM(level_2) AS level_2_total, STRFTIME('%Y-%m-01',  date(date / 1000, 'unixepoch')) as summary_date FROM record GROUP BY STRFTIME('%Y-%m-01', summary_date) ORDER BY summary_date DESC")
