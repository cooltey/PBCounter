...
interface RecordDao {
    ...
    @Query("SELECT SUM(level_1) AS level_1_total, SUM(level_2) AS level_2_total, date as summary_date, STRFTIME('%Y-%m-01',  date(date / 1000, 'unixepoch')) as group_date FROM record WHERE user_id IN (:userId) GROUP BY group_date ORDER BY group_date DESC")
    ...
    @Query("SELECT SUM(level_1) AS level_1_total, SUM(level_2) AS level_2_total, date as summary_date, STRFTIME('%Y-%m-01',  date(date / 1000, 'unixepoch')) as group_date FROM record WHERE user_id IN (:userId) AND group_date = :date")
    fun getMonthlySummaryById(userId: Long, date: String): LiveData<Summary>
